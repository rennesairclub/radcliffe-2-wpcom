...
= 24 September 2018 =
* Remove Site Title from Headstart annotations for the different verticals, since it overwrites what's added during signup.
...
= 6 July 2018 =
* Minor CSS updates to fix W3C validation issues.
...
* Make hover transparency more consistent across posts.
* Remove colour annotations for form inputs and text areas.
...
= 7 March 2018 =
* improvements to contact form styles.
...
= 19 December 2017 =
* Correcting where caption opacity is set, so it doesn't also affect the captioned image.
...
= 8 November 2017 =
* Switching direction contact labels are hidden offscreen for RTL languages, to prevent side-scrolling on mobile.
* Updating flyout positioning to use a percentage, rather than pixels, so it will adjust when font sizes are changed.

= 2 November 2017 =
* Fixing CTA alignment issue in IE 11.

= 20 October 2017 =
* Fixed JS dependencies on a customize_controls_enqueue_scripts enqueue that caused a problem with the close button in the Customizer.

= 19 October 2017 =
* Fix bug where unclosed symbol tag in SVG file caused IE/Edge browsers to show the Hours icon along with multiple other icons.

= 18 October 2017 =
* Update wording on 404 page to reflect lack of presence of links/widgets below, only a search field.
* Update color annotations to improve contrast of WooCommerce elements.

= 17 October 2017 =
* Update background color behind the Featured Propmpt to grey.

= 12 October 2017 =
* Check screen size to determine whether to set a min height or a fixed height on the hero area. The previous commit was causing the hero area height to be too tall and its max-height in the CSS was being ignored.
...
= 10 October 2017 =
* Add localized title tags to SVG icons for better accessibility/context.
...
* Fix case where long titles/descriptions/too much content in the hero area breaks the layout and overflows on mobile screens by setting a minimum height (rather than a fixed height) on the area.
...
* Fix visited link colors for the Colorful style pack in the social menu in Firefox.
